...
if /I not "%Arch%"=="x64" (
	echo ERROR: Unsupported architecture "%Arch%". FieldWorks patch builds are x64-only.
	exit /b 1
)
...
(
	...
	candle.exe -arch %Arch% -dBuildArch=%Arch% -dAppName=%AppName% -dVersionNumber=%PatchVersion% -dProductCode=%PRODUCTIDGUID% -dManufacturer=%Manufacturer% -dSafeManufacturer=%SafeManufacturer% -dPatchBaseline=%Baseline% -dPatchFamily=%Family% patch.wxs
) && (
